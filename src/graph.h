...
class Graph
{
    private:
        ...
        void add_node_and_edges(double *edgelist, int *nuclei, double *score);
    ...
    public:
        ...
        Graph(double *edgelist, int *nuclei, double *score, const options& opts): opts(opts)
        {
            g=new graph_t;
            add_node_and_edges(edgelist, nuclei, score);
        }
        ...
        void gather_labels_node(vector_double& v, double& c, vertex_descriptor p);
        ...
        vector_double solve_dc_nodewise(vertex_descriptor p,const vector_double& y_gt);
...
void Graph::gather_labels_node(vector_double& v_p, double& c_p, vertex_descriptor p)
{
...
vector_double Graph::solve_dc_nodewise(vertex_descriptor p,const vector_double& y_gt)
{
...
void Graph::schedule_nodes(std::vector<vertex_vector>& J_all)
{
	vertex_vector J;
	...
	const int NUM_PROCESSORS=(opts.use_parallel_processing)?opts.num_processors:1;
